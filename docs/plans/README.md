...
| 027 | Host-supplied app-scoped cache directory on `DotnetRuntimeContext` | P1 | M | — | TODO — planned; blocks 022, because user-wide cache paths must not ship. Adds a versioned app-directories struct to the runtime-context ABI and exposes `context.CacheDirectory`, mirroring upstream `AppContext`. One open reviewer decision: whether to extract the duplicated runtime-context ABI into a shared native header first. |
