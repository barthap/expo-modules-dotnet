...
| 027 | Host-supplied app-scoped directories on `DotnetRuntimeContext` | P1 | L | — | IN PROGRESS — revised 2026-07-25; risk MED-HIGH, because the private native↔managed create signature changes across four adapters. Blocks 022 and unblocks 024 with no further ABI work. Adds a versioned app-directories struct to the runtime-context ABI and exposes `context.CacheDirectory` and `.PersistentFilesDirectory`, mirroring upstream `AppContext`. Extracts the four duplicated runtime-context declarations into `native/include/expo_dotnet_host.h` first (operator-approved 2026-07-25), then renames the create entry point to v2 so stale adapter/host pairings fail resolution. Windows adapter builds are not runnable from a macOS executor, so this row stays non-DONE until they pass. |
