...
2026-07-25 at `6634abbb`; updated 2026-08-27 for plan 022 completion). Rows 022,
024, and 027 are current as of that update; the rest were last swept on
2026-07-20.
Execute in
...
| 022 | `expo-asset-dotnet` for Windows and macOS | P1 | M | 027 | DONE — standalone facade, strict request validation, app-scoped cache, offline HTTP service, generated binding, and cancellation cleanup implemented. Package managed tests: 43; full managed suite: 760; facade tests: 2; autolinking tests: 102. No tests skipped; typecheck, formatting, scope, and policy checks pass. |
...
| 023 | `expo-constants-dotnet` for Windows and macOS | P1 | M | — (022 recommended first) | DONE — six typed read-only constants, an extensible v3 host context, and native versions from macOS bundles or Windows package identity. The full managed suite passes 770 tests on Linux, macOS, and Windows with no skips; package JS and autolinking suites, typechecks, formatting, both macOS loader runs, and packaged Windows HostFXR/NativeAOT runs pass. Direct unpackaged Windows example startup was not verified; the no-package branch was reviewed in native source and null metadata is covered downstream by tests. |
