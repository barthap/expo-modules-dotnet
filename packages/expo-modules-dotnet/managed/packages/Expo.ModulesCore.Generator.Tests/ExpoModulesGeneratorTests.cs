...
public sealed class ExpoModulesGeneratorTests
{
  ...
  public void GeneratorEmitsDeterministicProviderForAssembly()
  {
    ...
    Assert.Contains(
        "public static void Register(global::Expo.ModulesCore.RuntimeSession session, global::Expo.JSI.JavaScriptObject modules)",
        source
    );
    Assert.DoesNotContain("public static void Register(global::Expo.JSI.JavaScriptRuntime runtime", source);
    Assert.Contains("ModuleRegistry.DefineModule(session.Runtime, modules, \"Math\")", source);
    Assert.Contains("session.GetOrCreateModule(\"Math\", static () => new global::Expo.TestModules.MathModule())", source);
  }
  ...
  public void GeneratorEmitsDefaultAndExplicitFunctionNames()
  {
    ...
    Assert.Contains("ModuleRegistry.DefineModule(session.Runtime, modules, \"Math\")", source);
    Assert.Contains("session.GetOrCreateModule(\"Math\", static () => new global::Expo.TestModules.InternalMathModule())", source);
