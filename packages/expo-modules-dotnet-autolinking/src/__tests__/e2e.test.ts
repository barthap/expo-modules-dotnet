...
import { resolvePlatform as resolveLinkPlatform } from '../commands/linkCommand';
...
describe('dotnet autolinking e2e', () => {
  it('accepts mobile link platforms and rejects unsupported platforms', () => {
    expect(resolveLinkPlatform('ios')).toBe('ios');
    expect(resolveLinkPlatform('android')).toBe('android');

    expect(() => resolveLinkPlatform('linux')).toThrow(
      '[expo-modules-dotnet-autolinking] Unsupported link platform linux. Use macos, windows, ios, or android.'
    );
  });
