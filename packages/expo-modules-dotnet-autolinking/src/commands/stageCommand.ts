...
type StagePlatform = 'macos' | 'windows';
...
function resolvePlatform(platform: StagePlatform | undefined): StagePlatform {
  if (platform === undefined || !supportedPlatforms.has(platform)) {
    throw new Error(
      `[expo-modules-dotnet-autolinking] Unsupported stage platform ${platform}. Use macos or windows.`
    );
  }
